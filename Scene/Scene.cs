...
public sealed class Scene
{
    ...
    /// <summary>Recalculates group pivots from a changed node's parent up to the scene root.</summary>
    public static void RecalculatePivotsToRoot(SceneObjectGroup? start)
    {
        for (SceneObjectGroup? current = start; current != null; current = current.Parent)
            current.RecalculatePivot();
    }

    /// <summary>Returns the highest root-level ancestor for a recursive scene node.</summary>
    public SceneObjectGroup? HighestAncestorById(int id)
    {
        SceneObjectGroup? group = GroupById(id);
        if (group == null)
            return null;
    ...
    /// <summary>
    /// Wraps one or more existing root objects in a new selectable parent node.
    /// This is used by the composer so every inserted asset appears as one top-level
    /// scene node while preserving the importer's internal hierarchy below it.
    /// </summary>
    public SceneObjectGroup WrapRootGroups(IEnumerable<int> rootIds, string name)
    {
        HashSet<int> requested = rootIds.ToHashSet();
        List<SceneObjectGroup> roots = ObjectGroups
            .Where(group => requested.Contains(group.Id))
            .ToList();

        if (roots.Count == 0)
            throw new InvalidOperationException("No root scene objects were supplied for grouping.");

        SceneObjectGroup wrapper = new(nextGroupId++, name, selectable: true);
        int insertionIndex = roots.Select(root => ObjectGroups.IndexOf(root)).Where(index => index >= 0).DefaultIfEmpty(ObjectGroups.Count).Min();
    ...
    public void DeleteGroup(int id)
    {
        ...
        SceneObjectGroup? formerParent = group.Parent;
        if (formerParent != null)
            formerParent.RemoveChild(group);
        else
            ObjectGroups.Remove(group);

        RecalculatePivotsToRoot(formerParent);
        RebuildWorldGeometry();
    }
