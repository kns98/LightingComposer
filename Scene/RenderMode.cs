 ...
 *
 * `RenderMode` makes a closed set of choices compiler-visible instead of passing loosely related integers or
 * strings. Code that switches over `Lit`, `Unlit`, `NormalDebug`, `UvDebug`, `MaterialDebug`, `LightDebug`,
 * `Wireframe`, `BoundingBox`, `Depth` is where the behavioral meaning of each choice is implemented.
 */
